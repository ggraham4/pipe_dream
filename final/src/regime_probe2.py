...
def main():
    ...
    ap.add_argument("--cash-ticker", default="SGOV,BIL,SHV",
                     help="Comma-separated preference order for the CASH LEG. "
                          "Gabe's refinement (2026-09-08): hold a real, tradable "
                          "T-bill ETF rather than an abstract rate. SGOV is the "
                          "natural choice but only lists from 2020; BIL goes back "
                          "to 2007 and covers this whole sample, SHV to 2007 too. "
                          "Whichever is found first is realized through the SAME "
                          "execution path as every equity leg -- next-open entry, "
                          "same horizon, same costs -- so 'go to cash' is priced "
                          "as an actual trade you could place.")
    ap.add_argument("--cash-yield", type=float, default=2.0,
                     help="Annualized %% fallback for windows the cash ETF does not "
                          "cover (e.g. pre-inception). Only used where the ETF is "
                          "missing; the script reports how many windows each source "
                          "supplied.")
    ...
    cash_fallback = (1 + args.cash_yield / 100) ** (FORWARD_WINDOW / TRADING_DAYS) - 1
    ...
    # ---- the cash leg -----------------------------------------------------
    cash_px, cash_name = None, None
    for cand in [c.strip() for c in args.cash_ticker.split(",") if c.strip()]:
        for d in cw.PRICE_DIRS:
            fp = d / f"{cand}.csv"
            if fp.exists():
                cash_px = pd.read_csv(fp, usecols=["date", "open", "high", "low", "close"],
                                       parse_dates=["date"]).sort_values("date")
                cash_px = cash_px.reset_index(drop=True)
                cash_name = cand
                break
        if cash_px is not None:
            break
    if cash_px is None:
        print(f"cash leg: no ETF found among {args.cash_ticker}; using a flat "
              f"{args.cash_yield:.1f}% annualized for every window")
    else:
        print(f"cash leg: {cash_name} ({cash_px['date'].min().date()} to "
              f"{cash_px['date'].max().date()}), realized through the same "
              f"execution path as the equity legs")
    ...
    for tp in step_dates:
        ...
        if len(names) < 20:
            continue
        cpos = (realize_position(cash_px, tp, FORWARD_WINDOW, entry_lag=1,
                                  entry_at=args.entry_at, stop_pct=None,
                                  cost_bps=args.cost_bps)
                if cash_px is not None else None)
        W.append({"cash": cpos["net_return"] if cpos else None,
    ...
    n = len(W)
    cash_arr = np.array([w["cash"] if w["cash"] is not None else cash_fallback
                          for w in W])
    n_etf = sum(1 for w in W if w["cash"] is not None)
    print(f"cash returns: {n_etf}/{len(W)} windows from {cash_name or 'n/a'}, "
          f"{len(W)-n_etf} from the {args.cash_yield:.1f}% fallback; "
          f"mean {cash_arr.mean()*100:.3f}%/window "
          f"(range {cash_arr.min()*100:.3f}% to {cash_arr.max()*100:.3f}%)")
    ...
    for bname, bret in base.items():
        b = np.where(np.isfinite(bret), bret, cash_arr)
    ...
    json.dump({"n_windows": n, "cash_ticker": cash_name,
                "cash_mean_pct": float(cash_arr.mean() * 100),
