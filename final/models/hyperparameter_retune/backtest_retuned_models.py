"""
Rerun the walk-forward dollar backtest (equal-weight top-5 + calibrated-
decile Kelly optimizer) using the ACTUAL best hyperparameters found by
retune_all_hyperparameters.py, for both candidate model families, on all
three variants. This is the test that matters more than CV deviance/MAE --
those are proxy metrics, this is the real $ outcome.

Candidates per variant (from retune_all_hyperparameters_results.json):
  - Tweedie-retuned: power=1.1, alpha=0 for every variant (the CV winner
    was the same across all three -- see write-up)
  - GAM hurdle: lam/n_splines per variant's own CV winner (also nearly
    identical across variants: (100,6) for OLD, (20,10) for BASE_PIT/AUG_PIT)

GAM refits at each of the 7 walk-forward timepoints are capped at 150k
training rows before the ITM split (same cap as the CV sweep), since GAM
fit time scales badly and the training pool grows to 500k+ rows by the
later timepoints.

Run from final/:  python3 models/hyperparameter_retune/backtest_retuned_models.py
"""
import gc
import json
import os
...
GAM_TRAIN_CAP = 150_000

with open("models/hyperparameter_retune/results/retune_all_hyperparameters_results.json") as f:
    tuned = json.load(f)


def load_spy():
    df = pd.read_parquet("data/training/options_calls_training.parquet", columns=["act_symbol", "entry_date", "underlying_close_entry"])
    ...
    return spy_rows[["date", "close"]].drop_duplicates("date").sort_values("date").reset_index(drop=True)
...
def predict_tweedie(train, cohort, feats, power, alpha):
    ...
    return model.predict(Xc) - 1.0


def predict_gam(train, cohort, feats, lam, n_splines):
    if len(train) > GAM_TRAIN_CAP:
        train = train.sample(n=GAM_TRAIN_CAP, random_state=0)
    scaler = StandardScaler().fit(train[feats].to_numpy())
    Xtr = np.clip(scaler.transform(train[feats].to_numpy()), -5, 5)
    Xc = np.clip(scaler.transform(cohort[feats].to_numpy()), -5, 5)
    y_tr = train["return_ratio"].to_numpy()
    ...
    for i in range(1, len(feats)):
        terms = terms + s(i)
    try:
        gam = GammaGAM(terms, lam=lam, n_splines=n_splines, max_iter=50).fit(Xtr_itm, y_tr_itm)
    except Exception as e:
        print(f"      GAM fit failed: {e} -- falling back to Tweedie(1.4,0.001) for this timepoint", flush=True)
        return predict_tweedie(train, cohort, feats, 1.4, 0.001)
    p_itm = clf.predict_proba(Xc)[:, 1]
    mag = np.clip(gam.predict(Xc), 0, None)
    return p_itm * mag - 1.0


def run_backtest(name, path, extra_feats, predict_fn, model_label):
    print(f"\n{'='*70}\n{name} -- {model_label}\n{'='*70}", flush=True)
    ...
    for T in TIMEPOINTS:
        ...
        t0 = time.time()
        cohort["pred_pct_return"] = predict_fn(train, cohort, feats)
        ...
        row = dict(timepoint=T.date().isoformat(), old_return_pct=old_return_pct, new_return_pct=new_return_pct,
                   spy_return_pct=spy_return_pct, universe_avg_return_pct=universe_avg_return, pool_size=len(pool_df))
        rows_out.append(row)
        print(f"{T.date()}: OLD={old_return_pct:+.2%} | NEW={new_return_pct if pd.notna(new_return_pct) else float('nan'):+.2%} "
              f"| SPY={spy_return_pct:+.2%} | universe={universe_avg_return:+.2%} | fit={time.time()-t0:.1f}s", flush=True)
    ...
    summary = dict(
        variant=name, model=model_label,
    ...
    print(f"\n{name} {model_label} SUMMARY: OLD avg={summary['old_avg']:+.2%} std={summary['old_std']:.2%} "
          f"win={summary['old_win_vs_spy']}/{summary['n']} | NEW avg={summary['new_avg']:+.2%} "
          f"std={summary['new_std']:.2%} win={summary['new_win_vs_spy']}/{summary['n']}", flush=True)
    return results, summary


all_summaries = {}
VARIANTS = [
    ("OLD", "data/training/options_calls_training.parquet", []),
    ("BASE_PIT", "data/training/options_calls_training_pit.parquet", []),
    ("AUG_PIT", "data/training/options_calls_training_pit.parquet", FUND_FEATURES),
]

for vkey, path, extra in VARIANTS:
    tw = tuned[vkey]["tweedie_best"]
    results, summ = run_backtest(vkey, path, extra,
                                  lambda train, cohort, feats, p=tw["power"], a=tw["alpha"]: predict_tweedie(train, cohort, feats, p, a),
                                  f"Tweedie-retuned(power={tw['power']},alpha={tw['alpha']})")
    all_summaries[f"{vkey}_tweedie_retuned"] = summ
    results.to_csv(f"models/hyperparameter_retune/results/backtest_retuned_{vkey}_tweedie.csv", index=False)

    gb = tuned[vkey]["gam_best"]
    if gb and not gb.get("failed"):
        results, summ = run_backtest(vkey, path, extra,
                                      lambda train, cohort, feats, l=gb["lam"], n=gb["n_splines"]: predict_gam(train, cohort, feats, l, n),
                                      f"GAM(lam={gb['lam']},n_splines={gb['n_splines']})")
        all_summaries[f"{vkey}_gam"] = summ
        results.to_csv(f"models/hyperparameter_retune/results/backtest_retuned_{vkey}_gam.csv", index=False)

print(f"\n\n{'='*70}\nFINAL COMPARISON (retuned models vs each other)\n{'='*70}")
print(f"{'key':<25} {'OLD avg':>9} {'OLD std':>9} {'winOLD':>7} | {'NEW avg':>9} {'NEW std':>9} {'winNEW':>7}")
for k, s in all_summaries.items():
    print(f"{k:<25} {s['old_avg']:>8.2%} {s['old_std']:>8.2%} {s['old_win_vs_spy']:>4}/{s['n']:<2} | "
          f"{s['new_avg']:>8.2%} {s['new_std']:>8.2%} {s['new_win_vs_spy']:>4}/{s['n']:<2}")

with open("models/hyperparameter_retune/results/backtest_retuned_summaries.json", "w") as f:
    json.dump(all_summaries, f, indent=2, default=str)
print("\nSaved backtest_retuned_summaries.json")
