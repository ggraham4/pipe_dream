...
for exp in expirations:
    target = exp - pd.Timedelta(days=60)
...
pairs_df = pairs_df[(pairs_df["days_to_expiration"] >= 45) & (pairs_df["days_to_expiration"] <= 75)].reset_index(drop=True)
print(pairs_df["days_to_expiration"].describe(), flush=True)
pairs_df.to_parquet(OUT_DIR / "entry_expiration_pairs_60d.parquet")
