...
def calculateRelativeStrength(Ticker,
    ...
    # explicitly reindex SPY's momentum onto the ticker's own dates before
    # subtracting. Without this, pandas' automatic Date-alignment on
    # subtraction returns the UNION of both indices, silently producing a
    # longer result than the ticker's own data whenever the ticker's
    # history is shorter than SPY's, exactly what was breaking every
    # post-1995-IPO company in the last run.
    spy_momentum_aligned = spy_momentum.reindex(ticker_momentum.index)
    
    rel_strength = ticker_momentum - spy_momentum_aligned
    return rel_strength
...
def priceHistoryBatchChunked(tickers, start_date, end_date, chunk_size=100, pause_seconds=5):
    """
    Same as priceHistoryBatch, but splits a large ticker list into smaller
    chunks with a pause between requests, to avoid tripping Yahoo's rate
    limiter on large pulls (1000+ tickers).
    """
    from src.data_loading import priceHistoryBatch
    chunks = [tickers[i:i + chunk_size] for i in range(0, len(tickers), chunk_size)]
    frames = []
    for i, chunk in enumerate(chunks):
        print(f"  Pulling chunk {i+1}/{len(chunks)} ({len(chunk)} tickers)...")
        try:
            frames.append(priceHistoryBatch(chunk, start_date, end_date))
        except Exception as e:
            print(f"  Chunk {i+1} failed entirely: {e}")
